...
app.post('/api/classrooms', authenticateToken, requireLecturer, async (req, res) => {
    const { class_name, class_details } = req.body;
    ...
    try {
        ...
        const newClassroom = await Classroom.create({
            ...
            // classroom_code will be auto-generated by the beforeCreate hook
        });
        ...
        if (error.name === 'SequelizeUniqueConstraintError') {
            return res.status(409).json({ 
                message: 'Classroom code already exists. Please try again.' 
            });
        }
